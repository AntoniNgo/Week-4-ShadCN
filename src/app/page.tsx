    ...
    <div className="mx-auto flex w-full max-w-[976px] flex-1 flex-col gap-8 p-6">
      ...
      <section className="space-y-4">
        <div className="space-y-0">
          <p className="text-sm font-medium text-muted-foreground mb-0">Total balance</p>
          <h2 className="text-3xl font-bold tracking-tight mb-0">10.00 EUR</h2>
        </div>
        ...
        <ul className="divide-y divide-border rounded-lg border">
          {RECENT_TRANSACTIONS.map((tx) => (
            <li key={tx.id} className="flex items-center gap-4 px-4 py-3">
              ...
              <p className={`shrink-0 text-right font-medium ${tx.isCredit ? "text-primary-text" : ""}`}>
                {tx.amount}
              </p>
            </li>
          ))}
